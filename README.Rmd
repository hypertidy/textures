...
[![CRAN status](https://www.r-pkg.org/badges/version/textures)](https://CRAN.R-project.org/package=textures)
<!-- badges: end -->



The goal of textures is to generate quad mesh primitives from the compact
specification of a regular grid, its dimension and extent.  The original
motivation was to use texture mapping in graphics to work with images in
different coordinate systems and mapped onto arbitrary shapes.
  

We aim to illustrate texture mapping capability in rgl with *core techniques familiar to R users*, and with minimal *resort to specialist code*. When specialist code is required
...
* **quad_texture()** create a mesh3d object with 1 or more quads, and the texture coordinates and
...
* **break_mesh()** break the topology of a mesh (so that primitives can be free-floating, not
 tied to their neighbour's vertices)


See [design vignette](https://hypertidy.github.io/textures/articles/design.html). 
