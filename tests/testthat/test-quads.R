...
test_that("quad works, basic 1x1 canvas", {
  ...
  expect_warning(qt <- quad_texture(), "no texture file given")
  expect_warning(qty <- quad_texture(ydown = TRUE), "no texture file given")
  expect_warning(quad_texture(texture = "no-such-file.png"), "texture file given does not exist")
...
17L, 13L, 14L, 20L, 19L, 14L, 15L, 21L, 20L, 15L, 16L, 22L, 21L, 
16L, 17L, 23L, 22L, 17L, 18L, 24L, 23L, 19L, 20L, 26L, 25L, 20L, 
21L, 27L, 26L, 21L, 22L, 28L, 27L, 22L, 23L, 29L, 28L, 23L, 24L, 
30L, 29L), dim = c(4L, 20L))

test_that("quad works, 5x4 canvas", {

  ## quads are in cell order, x fastest (raster cell order when ydown)
  q1 <- quad(c(5, 4))
